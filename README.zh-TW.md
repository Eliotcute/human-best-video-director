...
由 [Eliot](https://x.com/shizhieliot) 建立。

一套面向內容創作者、新媒體營運、個人 IP、創辦人和小型團隊的中文編導 Skills。

它處理從「這個帳號應該做給誰看」，到「這一條具體怎麼拍」，再到「發布後下一條應該改什麼」的完整內容流程。你可以用它做帳號定位、個人 IP、選題判斷、標題與開頭、口播稿、拍攝方案和發布後複盤。
...
- **內容創作者和部落客**：把零散想法做成能發布的選題、開頭、文案和拍攝方案。
- **新媒體營運**：為新帳號定方向，安排首批內容，並根據真實數據做下一輪調整。
- **個人 IP 和知識型創作者**：從真實經歷、能力和觀點中找到可以長期表達的內容方向。
- **創辦人和品牌負責人**：把產品、業務與創業經歷轉成使用者願意理解和信任的內容。
- **不露臉或低預算創作者**：根據錄螢幕、實拍、資料、旁白和有限人手設計可執行方案。
- **正在訓練編導能力的人**：用真實任務反覆練習需求判斷、選題、畫面、節奏和素材使用。

如果你只想把一句話潤飾得更順，一般寫作工具已經足夠。這個專案更適合需要先判斷方向，再把內容真正做出來的人。

## 專案結構

專案由一個統一入口和八個專項 Skill 組成。統一入口負責理解需求、選擇這次需要的 Skill、保存已確認的資訊，並把各階段結果整合成一份最終交付；專項 Skill 各自只處理一個明確問題。
...
flowchart TB
    U(["使用者的內容需求"]) --> M["human-best-video-director<br/>統一入口<br/>理解需求 · 選擇路徑 · 整合驗收"]

    M -- "資訊不足" --> I["hbvd-intake<br/>需求釐清<br/>最多 3 個關鍵問題"]
    I -- "補齊關鍵資訊" --> M

    M -- "帳號與人物" --> A["hbvd-account<br/>帳號定位與起號<br/>目標使用者 · 關注理由 · 內容支柱"]
    M --> P["hbvd-ip<br/>個人 IP 與創辦人故事<br/>真實資格 · 信任來源 · 人物表達"]
    M -- "內容決策" --> T["hbvd-topic<br/>選題決策<br/>首選選題 · 真實證據 · 驗證條件"]
    M --> H["hbvd-hook<br/>標題與開頭<br/>標題 · 封面 · 第一句 · 第一畫面"]
    M -- "製作" --> S["hbvd-script<br/>腳本與製作<br/>文案 · 時間軸 · 鏡頭 · 拍攝清單"]
    M -- "回饋與成長" --> V["hbvd-review<br/>審稿與內容複盤<br/>事實 · 主要問題 · 下一輪改動"]
    M --> C["hbvd-training<br/>編導訓練<br/>真實任務 · 前後對照 · 驗收標準"]
...
實線表示常見的內容生產順序，虛線表示複盤或訓練後回到具體環節。使用者通常只需要使用統一入口，不需要自己安排這八個 Skill 的順序。
...
它會沿用你已經說過的事實、素材和限制，後面的 Skill 不應該重複問同樣的問題。
...
倉庫公開後，可以透過官方 Skills CLI 直接安裝。執行命令後，按提示選擇需要的 Skill 和 Agent：

```bash
npx -y skills add Eliotcute/human-best-video-director -g
```

`-g` 表示安裝到目前使用者的全域 Skills 目錄，不會在倉庫裡產生 `.agents/` 和 `skills-lock.json`。

也可以先只查看倉庫中可用的 Skill，不執行安裝：
...
Claude Code 原生使用 `SKILL.md`。九個目錄一起安裝後，系統才能根據你的問題選擇正確的功能。不同版本或團隊設定可能使用不同的 Skills 目錄，請以目前客戶端說明為準。

### 豆包

豆包電腦端具備自己的 `.skills` 工作區機制，檔案格式同樣以 `SKILL.md` 為核心。不要只匯入「人類最強編導」：必須讓九個 Skill 都出現在豆包的 Skills 目錄或「規則和技能」列表中，否則帳號定位、選題、腳本等功能可能無法被正確使用。

豆包版本和工作區形態更新較快，本倉庫暫不寫死本機目錄。透過 SkillHub 安裝後，請先確認列表中是否能看到 `human-best-video-director` 與八個 `hbvd-*` Skill；如果只看到一個，請把九個目錄分別匯入。

### TRAE

TRAE 支援專案級 `.trae/skills`。可把九個 Skill 目錄放進目前專案：
...
然後在 TRAE 的「規則和技能」中檢查九個 Skill 是否都能看到。TRAE 的使用方式可能與 Codex 的 `$skill-name` 不同，請按目前版本顯示的方式呼叫。如果只看到一個 Skill，帳號定位、選題和腳本等功能之間可能無法自動銜接。

### 相容性怎麼判斷

| 環境 | 目前判斷 | 使用前檢查 |
|---|---|---|
| Codex | 完整適配，已作為主要測試環境 | 九個 Skill 均出現在可用列表 |
| Claude Code | 完整適配 `SKILL.md` 與多 Skill 安裝 | 九個目錄一起複製到 Skills 目錄 |
| 豆包電腦端 | 支援同類檔案格式；安裝方式隨版本變化 | 列表中能看到全部九個 Skill |
| TRAE | 支援 `.trae/skills`，具體使用方式隨版本變化 | 列表中能看到全部九個 Skill，再測試一次起號或腳本任務 |
| 一般聊天模型 | 只能把內容當提示詞或知識檔案使用 | 不會自動選擇和銜接九個 Skill |

### 其他支援 `SKILL.md` 的 Agent

把倉庫中的九個 Skill 資料夾複製到該 Agent 的 Skills 目錄。安裝後確認九個 Skill 都能看到，並用一次完整任務檢查它能否從需求釐清繼續做到選題或腳本。如果客戶端不支援 `$skill-name` 語法，請使用它自己的呼叫方式。
